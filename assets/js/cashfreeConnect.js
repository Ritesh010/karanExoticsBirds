async function getCashfreePaymentLink(customerId, customerPhone, orderAmount) {
  ...
  try {
    ...
    const response = await fetch("http://68.183.247.116:3000/api/cashfree/generate-payment-id", {
      method: "POST",
      headers: {
...
async function getCashfreePaymentStatus(order_id) {
  ...
  try {
    const token = localStorage.getItem('token');
    const response = await fetch("http://68.183.247.116:3000/api/cashfree/check-payment", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        'Authorization': `Bearer ${token}`,
